...
## Owner answers to the open questions (2026-09-21)

| # | Decision | Why |
|---|---|---|
| D22 | **`rebut` is symmetric.** An authored rebut A→B implies B→A; `attackEdges` adds the reverse edge (`derived: true`, `derivedFrom`) unless both directions are authored. Undercut and undermine stay directed. Evaluator version 0.2.0 → 0.3.0. | Owner: "auto-add". Pollock rebutting defeat is defined on contrary conclusions, which is a symmetric relation; asking authors to write both lines only produced one-sided standoffs. |
| D23 | **No separate frame/slot schema.** TruthIR was merged into Logic IR upstream; the flat profile in `schema/truthir-profile-v0.1.schema.json` is the unit schema and no frame re-expression is planned. | Owner: "TruthIR was merged into LogicIR, no worries". |
| D24 | **Unit casting stays as built in T1:** concepts, claims and arguments cast to KSG's seeded prototypes; domain units (issue, position, source, evidence, hypothesis, theory, annotation, actor) keep a TruthApp `category_name` under `TruthUnit`. Domain prototypes are not requested from KSG. | Owner delegated; the split keeps logic primitives shared with KSG and domain vocabulary local, which is D2 and D15 applied. |
| D25 | **Comparisons are not added here.** D14 stands: no `Compare` node in this repo. The v0.0.2 proposal in `docs/OPEN_QUESTIONS.md` is KSG's to take or leave. | Owner: "don't fork". |
| D26 | **Formalise the doom core first.** The first AI-risk claims to get Logic IR and bound terms are the chain that reads "AI could destroy us all soon and there is no counterbalance": P3 (superintelligence within decades), P5 (misaligned power-seeking), P11 (everyone dies), then the no-counterbalance premises P4 (no alignment method), P9 (resists shutdown), P10 (one shot), P20 (no time to iterate). The skeptic replies that meet them (S2c, S13c, S18c) follow so the attack graph has IR on both ends. Modal, temporal and probabilistic content stays on the claim (D15); only the material core is formalised. | Owner's stated priority. These are the claims A1, A3, A4, A5 and A8 run through, so IR here exercises the most inference. |
