...
| T7 | ⬜ Formalise the doom core (D26): `logic:` + `terms` on P3, P5, P11, then P4, P9, P10, P20, then S2c, S13c, S18c, in `fixtures/ai-risk/source/case.json`; add the predicate concepts they need; regenerate `commits/0001-author-case.json` with `truth compile`; keep the `regrounded` branch's P3 revision consistent. Expect A1/A3/A4/A5 to move from `unresolved` to `valid`/`invalid` in `ksg-adapter.test.mjs` T4 and update that expectation honestly | closes when `npm test` is green with the new expectations and `truth ksg-push` reports ≥ 7 Propositions |
