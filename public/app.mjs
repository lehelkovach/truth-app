...
function renderGraph() {
  ...
  for (const e of r.edges) if (f[e.operator] !== false && have.has(e.from) && have.has(e.to)) edges.push({ data: { id: e.id + e.to, source: e.from, target: e.to, label: e.operator + (e.targetRef ? ` ${e.targetRef.split(':')[1]}` : ''), style: 'dashed', color: cssVar('--red') } });
...
function renderInspector(diag = null) {
  ...
  if (u) {
    ...
    if (u.kind === 'argument') {
      ...
      if (atk.length) parts.push(`<div class="small">attacked by: ${atk.map((e) => `<a data-sel="${e.from}">${esc(e.from.split(':')[1])}</a> (${esc(e.operator)}${e.targetRef ? ' ' + esc(e.targetRef.split(':')[1]) : ''}, ${LABEL_WORD[r.labels[e.from]]})`).join(', ')}</div>`);
